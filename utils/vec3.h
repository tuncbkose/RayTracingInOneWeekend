...
inline vec3 operator+(const vec3& u, double t){
    // added to make vec3::random(min, max) to have vector-scalar addition
    return vec3(u.e + t);
}
...
inline vec3 operator+(const vec3& u, double t){
    // added because of the SIMD version
...
vec3 vec3::random(double min, double max){
    return (max-min)*vec3::random() + min;
}
